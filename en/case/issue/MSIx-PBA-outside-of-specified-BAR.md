...
On multi-GPU hosts, virtual machines with passthrough PCI devices report this error on boot

## Symptoms

The virtual machine reports this error on boot

As shown in the example below!

`lspci -vvs 0000:2a:00.0`
```
00000:2a:00.0 3D controller: NVIDIA Corporation AD102GL [L20] (rev a1)
	...
	NUMA node: 0
	IOMMU group: 1
	Region 3: Memory at 800000000 (64-bit, prefetchable) [size=32M]  # Here only bar3 appears, there are no other bars
...
1. Add kernel parameter `pci=realloc` — Let the kernel reallocate PCI resources, which may solve the insufficient window problem
2. Adjust `MMIO High Base` + `MMIO High Size` in BIOS
3. Update BIOS